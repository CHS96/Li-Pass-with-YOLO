...
void swap(int a, int b, int* binary) {
    int tmp = binary[a];
    binary[a] = binary[b];
    binary[b] = tmp;
}

void convertForTwo(int* binary) {
    swap(0, 3, binary);
    swap(2, 3, binary);
    swap(0, 1, binary);  
}

void convertForFour(int* binary) {
    swap(2, 3, binary);
    swap(0, 1, binary);
    swap(0, 3, binary);
}

void convertForOne(int* binary) {
    swap(0, 3, binary);
    swap(1, 2, binary);
}
...
char convertToChar(int* binary) {
    int num = binary[3] * 8 + binary[2] * 4 + binary[1] * 2 + binary[0] * 1;
    ...
    int benchmark, int benchmark_layers)
{
    ...
    if(!prefix && !dont_show){
        int full_screen = 0;
        create_window_cv("Demo", full_screen, 1000, 1000);
    }
    ...
    while(flag1){
        ++count;
        {
            ...
            if (!benchmark && !dontdraw_bbox) draw_detections_cv_v3(show_img, local_dets, local_nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output, output);
	...
	if (flag2 && flag3){
           if (output[strlen(output)-3] == 'f' && output[strlen(output)-2] == 'f' && output[strlen(output)-1] == 'f') flag2 = 0;
        } else if (!flag2 && flag3){
           if (output[strlen(output)-1] != 'f') flag3 = 0;
        } else if (!flag2 && !flag3){
           if (output[strlen(output)-3] == 'f' && output[strlen(output)-2] == 'f' && output[strlen(output)-1] == 'f') flag1 = 0;
        }
    }

	char convertedMessage[BUF_SIZE] = "";
	char tmp0[BUF_SIZE] = "", tmp1[BUF_SIZE] = "", tmp2[BUF_SIZE] = "", tmp3[BUF_SIZE] = "";
	
	char preChar = output[0];
	int count2 = 0;
	tmp0[0] = output[0];
	for (int i = 1; i < strlen(output); i++)
	{
		if (preChar == output[i]) count2++;

		if (count2 > 2) {
			tmp0[strlen(tmp0)] = output[i];
	...
	preChar = tmp0[0];
	tmp1[0] = tmp0[0];
	for (int i = 1; i < strlen(tmp0); i++)
	{
		if (preChar != tmp0[i]) {
			tmp1[strlen(tmp1)] = tmp0[i];
	...
	printf(" Remove the front of 'F': %s \n", tmp1);
	...
	for (int i = 0; i < strlen(tmp1); i++)
	{
		...
		if (flag == 1) {
			tmp2[strlen(tmp2)] = tmp1[i + 1];
			tmp2[strlen(tmp2) + 1] = '\0';
		}
	}

	printf(" Remove the back of 'F' : %s \n", tmp2);
	...
	for (int i = 0; i < strlen(tmp2); i++)
	{
		...
		tmp3[strlen(tmp3)] = tmp2[i];
		tmp3[strlen(tmp3) + 1] = '\0';
	}

	printf(" Remove '0' : %s \n", tmp3);

	int location = tmp3[0] - '0';

	printf("Location Bit : %d \n", location);

	int binary[4] = { 0, };

	for (int i = 1; i < strlen(tmp3); i++) {

		switch (location) {
		case FOUR:
			convertToBinary(convertToNumber(tmp3[i]), binary);
			convertForFour(binary);
			convertedMessage[strlen(convertedMessage)] = convertToChar(binary);
			break;
		case TWO:
			convertToBinary(convertToNumber(tmp3[i]), binary);
			convertForTwo(binary);
			convertedMessage[strlen(convertedMessage)] = convertToChar(binary);
			break;
		case ONE:
			convertedMessage[strlen(convertedMessage)] = tmp3[i];
			break;
		case EIGHT:
			convertToBinary(convertToNumber(tmp3[i]), binary);
			convertForOne(binary);
			convertedMessage[strlen(convertedMessage)] = convertToChar(binary);
			break;
		}
	}
    ...
    convertedMessage[strlen(convertedMessage)] = '\0';
    printf(" ConvertedMessage : %s\n", convertedMessage);
    
    char out[BUF_SIZE] = "";
    strcat(out, "python3 test.py ");
    strcat(out, convertedMessage);
    system(out);
