...
bool cmpRadius(const Vertex & a, const Vertex & b) {
    if (b.radius > a.radius) return 0;
    else return 1;
}

bool cmpY(const Vertex & a, const Vertex & b) {
...
// Version : detector Demo
Mat* preProcessingDemo(Mat* &mat) {
    Mat& I = *mat;

    Mat tmp(I.rows, I.cols, CV_8UC1, Scalar(255));
    // ¿øº»ÀÌ¹ÌÁö H,S,V·Î ºÐž®
    Mat hsv_img, hsv[3];
    cvtColor(I, hsv_img, 40);
    split(hsv_img, hsv);

    int yy = I.rows;
    int xx = I.cols;

    // ÀÌ¹ÌÁö¿¡Œ­ H,S,V ±âÁØÀ» žžÁ·ÇÏŽÂ ºÎºÐ ÃßÃâ 
    vector<pair<int, int>> redLightVc;
    for (int y = 0; y < yy; ++y) {
        uchar* h = hsv[0].ptr<uchar>(y);
        uchar* s = hsv[1].ptr<uchar>(y);
        uchar* v = hsv[2].ptr<uchar>(y);
        for (int x = 0; x < xx; ++x) {
            if (h[x] <= 20) {
                redLightVc.push_back({ y, x });
            }
        }
    }

    for (int i = 0; i < (int)redLightVc.size(); ++i) {
        tmp.at<uchar>(redLightVc[i].first, redLightVc[i].second) = 0;
    }

    // Red ¿µ¿ªÀ» ÀÌ·çŽÂ ¿µ¿ª ÃßÃâ
    vector<vector<Point>> contours;
    vector<Vec4i> hierarchy;
    findContours(tmp, contours, hierarchy, 3, 2, Point(0, 0));

    // ÄÁÅõŸî·Î ÃßÃâÇÑ ¿µ¿ªÀÇ ÁßœÉ, ¹ÝÁöž§ ÃßÃâ
    ...
    vector<Vertex> vertexVc;
    for (int i = 0; i < contours.size(); i++) {
        vertexVc.push_back({ centers[i], radius[i] });
    }

    sort(vertexVc.begin(), vertexVc.end(), cmpRadius);

    // 4°³ÀÇ ²ÀÁþÁ¡ ÁÂÇ¥ ÃßÃâ
    vector<Vertex> vertex;
    for (int i = 1; i <= 4; ++i) {
        vertex.push_back(vertexVc[i]);
    }

    sort(vertex.begin(), vertex.end(), cmpY);

    // ²ÀÁþÁ¡ÀÇ À§Ä¡¿¡ µû¶ó ÈžÀüÇÒ °¢µµ, ±âÁØÁ¡, º¯ÀÇ ±æÀÌ ±žÇÔ 
    double len = sqrt(pow(vertex[0].center.x - vertex[1].center.x, 2.0) + pow(vertex[0].center.y - vertex[1].center.y, 2.0));
    double angle = getAngle(vertex[0].center.x, vertex[0].center.y, vertex[1].center.x, vertex[1].center.y);
    ...
    if (angle < 0) {
        angle = -(angle + 90);
        startY = vertex[0].center.y;
        startX = vertex[0].center.x - len;
    }
    else {
        angle = 90 - angle;
        startY = vertex[0].center.y;
        startX = vertex[0].center.x;
    }

    // ROI ¹üÀ§ Á¶Àý
    startY -= diffY * 1.6; startX -= diffX * 1.6;
    len += 2.3 * (diffY + diffX);

    // ±žÇÑ °¢µµ·Î ÁßœÉ±âÁØ ÀÌ¹ÌÁö ÈžÀü
    Mat rotation = getRotationMatrix2D(Point2f(I.rows / 2, I.cols / 2), angle, 1);
    warpAffine(I, I, rotation, Size(I.cols, I.rows));

    // ROI ¿µ¿ª ÃßÃâ
    //Rect rect(startX, startY, len, len);
    //I = I(rect);
...
extern "C" void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output, char* output)
{
    try {
